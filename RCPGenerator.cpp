...
static constexpr size_t N_STEPS      = 500000;         // Max optimization steps
...
int main(int argc, char** argv) {
    ...
    if (Ndim == 3){ alpha = 0.005; alpha_max = 0.005;}
    ...
    for (size_t step = 1; step <= N_steps; ++step) {
        ...
        if ((step > 500 && mu_flag == 1)) {
            ...
            if (delta_XX < 5e-6 || step - phi_max.second > 3500 || (step > std::max(280000, static_cast<int>(N / 3)))) {
                mu    /= 10.0;
                alpha /= 2.0;
                mu_flag = 0;
                mu_change = step;
        ...
        if (mu_flag == 0 && step == 300000){alpha = alpha/10;}
        if (mu_flag == 0 && step == 320000){alpha = alpha/10;}
        if (mu_flag == 1 && step == 380000){alpha = alpha/10;}
        ...
        if (step > 1100){
            ...
            if ((mu_flag == -1 &&
                ...
                max_min_dist > 1e-16 && 
                step > mu_change + 250) || (delta_XX < 1E-7)) {
